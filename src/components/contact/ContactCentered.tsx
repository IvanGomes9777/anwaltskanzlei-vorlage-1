      ...
      <div className="container-content py-24 md:py-28">
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-5 font-serif text-4xl font-semibold leading-tight text-navy md:text-5xl">
              {t('heading')}
            </h2>
        ...
        <Reveal delay={0.1}>
          <div className="mx-auto mt-12 max-w-xl rounded-2xl border border-navy/10 bg-white p-8">
            <ContactForm />
          </div>
        </Reveal>

        <Reveal delay={0.18}>
          <div className="mx-auto mt-10 flex max-w-2xl flex-wrap justify-center gap-x-12 gap-y-4 border-t border-navy/12 pt-8 text-center">
            {info.map((it) => (
              <div key={it.t}>
                <p className="text-xs uppercase tracking-wide text-gold-600">{it.t}</p>
                <p className="mt-1 text-sm text-navy/75">{it.v}</p>
              </div>
            ))}
          </div>
        </Reveal>
