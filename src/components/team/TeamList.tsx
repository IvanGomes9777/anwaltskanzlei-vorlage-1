      ...
      <div className="container-content py-24 md:py-28">
        <Reveal>
          <div className="flex flex-col gap-4 border-b border-navy/15 pb-8 md:flex-row md:items-end md:justify-between">
            ...
            <p className="max-w-sm text-base text-navy/60">{t('intro')}</p>
          </div>
        </Reveal>
        ...
        <div>
          {members.map((m, i) => (
            <Reveal key={m.name} delay={i * 0.06}>
              <div className="group flex items-center gap-6 border-b border-navy/12 py-6">
                ...
                <div className="flex flex-1 flex-col gap-1 md:flex-row md:items-center md:justify-between">
                  <div>
                    <h3 className="font-serif text-xl font-semibold text-navy transition-colors group-hover:text-gold-600">
                      {m.name}
                    </h3>
                    <p className="text-sm text-navy/55">{m.role}</p>
                  </div>
                  <p className="text-sm font-medium text-navy/70">{m.area}</p>
                </div>
