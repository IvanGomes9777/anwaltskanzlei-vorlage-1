    ...
    <section className="bg-[#728690]">
      <div className="container-content py-24 md:py-28">
        <Reveal>
          ...
          <p className="mt-5 max-w-xl text-lg text-black/65">{s('intro')}</p>
        </Reveal>

        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((it, i) => (
            <Reveal key={it.n} delay={i * 0.08}>
              <article className="group relative flex h-full flex-col rounded-xl border border-black/10 bg-[#728690] p-7 transition-all hover:-translate-y-1 hover:border-white/40 hover:shadow-[0_18px_40px_-24px_rgba(15,27,45,0.4)]">
                <span className="absolute left-0 top-7 h-8 w-px bg-white transition-all group-hover:h-12" />
                <span className="font-serif text-sm text-black">{it.n}</span>
                ...
                <p className="mt-5 text-xs uppercase tracking-wide text-black/40">
                  {it.tags}
                </p>
              </article>
            </Reveal>
